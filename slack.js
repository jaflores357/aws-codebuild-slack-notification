...
functions.handler = function(event, context) {
    ...
    const icon = {
        'IN_PROGRESS': ':neutral_face:',
        'FAILED': ':sob:',
        'SUCCEEDED': ':wink:'
    }

    // If failed.  Get logs path.
    var text = icon[status] + ' Build started by '+initiator+' - *Project*: '+project+' :: *Status*: '+status;

    if (status === 'SUCCEEDED') {
        text += ' *Build*: '+event.detail['additional-information']['build-number']
    }
    if (status === 'FAILED') {
        text += ' *Build*: '+event.detail['additional-information']['build-number']
        text += '\n';
